...
class MW(QtWidgets.QMainWindow):
    ...
    def __init__(self,mode = 'standard', headless= False, old = False, borstel = False):
        ...
        if mode == 'directional':
            self.ap = AnalyserPanel(mode = 'directional', headless = headless,old = old, borstel = borstel)
        
        else:
            self.ap = AnalyserPanel(headless = headless,old = old,borstel = borstel)
...
class AnalyserPanel(QWidget):
    ...
    def __init__(self,mode = 'standard', headless = False, old = False, borstel = False):
        ...
        self.autolaunch.overridet0flag = True
    ...
    def remove_vesicles_which_moved(self,dictionary,threshold = -0.07,frames =None):
        ...
        print('no of vesicle i series before removal',len(self.analyser.rffitrace))
        for i in range(0,len(tbk)):
            ...
            try: 
                ...
                continue
        print('no of vesicle i series after removal',len(self.analyser.rffitrace))
    ...
    def run_just_analysis(self):
        ...
        print('t0 frame number now', self.analyser.t0frameNo)
        
        self.analyser.get_clips_alt()
        self.analyser.classify_clips()
        self.analyser.analyse_frames(int(self.AControl.tmaxselector.currentText()))
        self.analyser.extract_background(int(self.AControl.tmaxselector.currentText()))
        self.analyser.subtract_background(sigma = 1)
        ...
        if self.has_frames:
            try:
                if multividflag:
                    ...
                    if self.manual==True:
                        traps,labels = self.analyser.get_traps(self.manual_trap_frame)
                    else:
...
if __name__ == '__main__':
    ...
    mw = MW(mode = 'directional', headless = headless, old = old, borstel = borstel)
